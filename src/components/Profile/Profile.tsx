...
export default function Profile({ className, isEdit = true }: Readonly<Props>) {
  const { isEditProfileActive, setIsEditProfileActive } = useContext(AppContext)
  const profile = useSelector((state: RootState) => state.user.profile)

  const images = profile?.photos?.filter((Url) => Url !== 'https://localhost:7251/Uploads/')
  const userName = profile?.fullName
  const age = profile?.age
  const passions = profile?.passion
  const languages = profile?.languages
  const liveAt = profile?.liveAt
  const company = profile?.company
  const jobTitle = profile?.jobTitle
  const school = profile?.school
  let basics: BasicsType = {
    zodiac: null,
    education: null,
    futureFamily: null,
    vacxinCovid: null,
  ...
  if (profile) {
    const { zodiac, education, futureFamily, vacxinCovid, personality, communication, loveLanguage } = profile
    const { pet, alcolhol, smoke, workout, diet, socialMedia, sleepHabit } = profile
    basics = { zodiac, education, futureFamily, vacxinCovid, personality, communication, loveLanguage }
    lifeStyles = { pet, alcolhol, smoke, workout, diet, socialMedia, sleepHabit }
  }
  ...
  useEffect(() => {
    ...
    if (divElement && divOverlayElement) {
      ...
      const divOverlayStyle = {
        ...
        background: 'linear-gradient(to bottom,transparent,rgb(240 242 244) 20%)'
      }
  ...
  return (
    <>
      {isEdit && (
        <div ref={divOverlayRef} className='absolute -translate-y-full z-20 h-[65px] flex items-center justify-center '>
          <button
            style={{ backgroundImage: 'linear-gradient(to top right, #fd267a, #ff6036)' }}
            onClick={navigateToEditProfile}
            className='text-lg opacity-60 w-[80%] z-20 h-[50px] px-2 py-1 rounded-full text-white hover:opacity-100'
          >
            Edit Info <span>(13% complete)</span>
          </button>
        </div>
      )}
      <div
        className={`${className} relative bg-white ${
          isEdit ? 'h-[600px] w-[360px] rounded-lg' : 'h-full w-[360px]'
        }  overflow-auto hide-scrollbar shadow-lg `}
        ref={divRef}
      >
        <div className='mb-20'>
          ...
          <div className='w-full flex justify-start items-center px-3 py-4'>
            <h1 className='text-3xl line-clamp-1'>{userName}</h1>
          ...
          <div className='border-t border-[#d4d8de] w-full' />
          <div className='w-full flex justify-start items-center px-3 pt-4'>
            <div className='flex items-center bg-[#c0dbd1] px-3 py-2 rounded-xl mb-3'>
              <img className='w-5 h-5' src='/assets/images/relationship_intent_tada.png' alt='' />
          ...
          {Array.isArray(passions) && (
            <div className='w-full  px-3 pt-4 text-[#21262E]'>
              ...
              <div className='flex flex-wrap'>
                {passions.map((item) => (
                  <span key={uuid()} className='py-1 px-3 mx-2 my-2 border-[#7c8591] border rounded-full'>
                    {item}
                  </span>
                ))}
              </div>
